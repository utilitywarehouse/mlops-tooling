...
class Timeseries:
    ...
    def prepare_dataset(self) -> pd.DataFrame:
        ...
        flattened_df = (
            self.df.drop(columns=columns_to_drop).dropna().reset_index(drop=True)
        )
    ...
    def add_date_info(self) -> pd.DataFrame:
        ...
        if self.date_features:
            for date_feat_name, date_feat_func in self.date_features.items():
                self.df = self.get_date_feature(
                    self.df, self.date_col, date_feat_name, date_feat_func
                )
    ...
    def add_target_lags(self) -> pd.DataFrame:
        ...
        for lag in self.lags:
            if self.group_cols:
                self.df[f"{self.target_col}_lag_{lag}"] = self.df.groupby(
                    self.group_cols
                )[self.target_col].shift(lag)
                if lag > 1:
                    self.df[f"{self.target_col}_roll_{lag}"] = self.df.groupby(
                        self.group_cols
                    )[self.target_col].apply(lambda v: v.shift(1).rolling(lag).mean())
            else:
                self.df[f"{self.target_col}_lag_{lag}"] = self.df[
                    self.target_col
                ].shift(lag)
                if lag > 1:
                    self.df[f"{self.target_col}_roll_{lag}"] = (
                        self.df[self.target_col].shift(1).rolling(lag).mean()
                    )
    ...
    def add_covariate_lags(self) -> pd.DataFrame:
        ...
        for covariate in self.past_covariates:
            for lag in self.covariate_lags:
                if self.group_cols:
                    self.df[f"{covariate}_lag_{lag}"] = self.df.groupby(
                        self.group_cols
                    )[covariate].shift(lag)
                else:
    ...
    def add_covariate_leads(self) -> pd.DataFrame:
        ...
        for covariate in self.future_covariates:
            for lead in self.covariate_leads:
                if self.group_cols:
                    self.df[f"{covariate}_lead_{lead}"] = self.df.groupby(
                        self.group_cols
                    )[covariate].shift(-lead)
                else:
                    self.df[f"{covariate}_lead_{lead}"] = self.df[covariate].shift(
                        -lead
                    )
    ...
    def get_date_feature(
        ...
        date_col: str,
        date_feature: str,
        date_feature_func: Optional[str] = None,
    ):
        """
        ...
        if date_feature == "week_of_month":
            ...
            week_of_month_temp_df = week_of_month_temp_df.drop_duplicates().reset_index(
                drop=True
            )

            week_of_month_temp_df["week_of_month"] = (
                week_of_month_temp_df.groupby(["year", "month"])
                .rank(method="first", ascending=True)
                .astype(int)
            )

            df = df.merge(
                week_of_month_temp_df,
                left_on=["year", "month", "week_start_date"],
                right_on=["year", "month", "week_start_date"],
            )
            ...
            weeks_in_month = (
                df.groupby(["year", "month"])[["week_of_month"]]
                .max()
                .reset_index()
                .rename(columns={"week_of_month": "no_weeks"})
            )

            df = df.merge(
                weeks_in_month, left_on=["year", "month"], right_on=["year", "month"]
            )
    ...
    def add_fourier_harmonics(self, seasonal_order, seasonal_period):
        time_df = self.df[[self.date_col]].drop_duplicates().reset_index(drop=True)
        n = time_df.shape[0]
        ...
        column_names = [
            f"fourier_period_{seasonal_period}_cos_{i+1}" for i in range(seasonal_order)
        ] + [
            f"fourier_period_{seasonal_period}_sin_{i+1}" for i in range(seasonal_order)
        ]

        fourier_df = pd.DataFrame(fourier_series, columns=column_names)

        time_df = pd.concat([time_df, fourier_df], axis=1)

        self.df = self.df.merge(time_df, left_on=self.date_col, right_on=self.date_col)
