...
def bootstrap_prediction_interval(
    model, x_val: pd.DataFrame, y_val: pd.DataFrame, n: int = 100, alpha: float = 0.05
):
    """
    ...
    bootstrap = np.asarray(
        [np.random.choice(residuals, size=residuals.shape) for i in range(n)]
    )
...
def rmse_prediction_interval(
    model, x_val: pd.DataFrame, y_val: pd.DataFrame, alpha: float = 0.05
):
    """
